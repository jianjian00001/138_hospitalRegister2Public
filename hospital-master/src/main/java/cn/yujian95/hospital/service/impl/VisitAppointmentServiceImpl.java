...
public class VisitAppointmentServiceImpl implements IVisitAppointmentService {
    ...
    @Override
    public Map<String, Object> getAppointmentBySpecial() {
        List<StaticsVo> staticsVos = appointmentMapper.getAppointmentBySpecial();
        List<String> list  = staticsVos.stream().sorted((o1, o2) -> {
                    if((o1.getValue() - o2.getValue()) < 0){
                        return 1;
                    }else {
                        return -1;
                    }})
                .map(StaticsVo::getName).collect(Collectors.toList());
        List<Integer> list2  = staticsVos.stream().sorted((o1, o2) -> {
                    if((o1.getValue() - o2.getValue()) < 0){
                        return 1;
                    }else {
                        return -1;
                    }})
                .map(StaticsVo::getValue).collect(Collectors.toList());
        Map<String, Object> map = new HashMap<>();
        map.put("name",list);
        map.put("value",list2);
        map.put("data",staticsVos);
        return map;
    }

    @Override
    public List<StaticsVo> qryUserStatisGroupBySex() {
        List<UserMedicalCard> userMedicalCards = userMedicalCardMapper.selectByExample(new UserMedicalCardExample());
        List<StaticsVo> list = new ArrayList<>();
        Map<Integer, Long> collect = userMedicalCards.stream().collect(Collectors.groupingBy(UserMedicalCard::getGender, Collectors.counting()));
        collect.entrySet().stream().forEach(str ->{
            StaticsVo sexVo = new StaticsVo();
            if(1== str.getKey()){
                sexVo.setName("男");
            }else {
                sexVo.setName("女");
            }
            sexVo.setValue(str.getValue().intValue());
            list.add(sexVo);
        });
        return list;
    }
